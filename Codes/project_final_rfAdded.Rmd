...
plot(bookMod$amtuponorders)
plot(bookMod$qtyuponorders)
plot(bookMod$amtuponqty)
...
#Random Forest Models to select important variables on book category
```{r}
#helper function to get the accuracy and root mean square error of model predictions
...
#train test split based on given data, this is a dataset without new variables added 
booktrain = read.csv("booktrain.csv")
btrain = merge(book, booktrain, by = "id")
btrain = subset(btrain, select = -c(logtargamt.y,X) )
btest = merge(book, booktest, by = "id")
btest = subset(btest, select = -c(logtargamt.x, X))

#random forest for variable importance 
library(caret)
ctrl = trainControl(method="cv", number = 5) 
class.rf = train(as.factor(successFlag) ~ ., data = subset(btrain, select = -c(id, logtargamt.x)), method = "rf", trControl = ctrl, tuneGrid = expand.grid(mtry = c(5, 10, 20, 30, 50)))
rf.accy = accy(btest$successFlag, predict(class.rf, newdata=btest))
#accuracy
rf.accy #0.9618
...
class.rf2 = train(as.factor(successFlag) ~ ., data = subset(btrain_cat, select = -c(id, logtargamt.x)), method = "rf", trControl = ctrl, tuneGrid = expand.grid(mtry = c(5, 10, 20, 30, 50)))
rf.accy2 = accy(btest$successFlag, predict(class.rf2, newdata=btest))
...
class.rf0 = train(as.factor(successFlag) ~ ., data = subset(btrain, select = c(recency, frequency, amount, tof, successFlag)), method = "rf", trControl = ctrl, tuneGrid = expand.grid(mtry = c(1,2,3,4)))
rf.accy0 = accy(btest$successFlag, predict(class.rf0, newdata=btest)) 
rf.accy0
...
regre.rf = train(logtargamt.x ~ ., data = subset(bregre, select = -c(id, successFlag)), method = "rf", trControl = ctrl, tuneGrid = expand.grid(mtry = c(5, 10, 20, 30, 50)), importance = T)
rf.rmse.regre = rmse(bregre_test$logtargamt.y, predict(regre.rf, newdata=bregre_test))
rf.rmse.regre 
#variable important of the regression
varImp(regre.rf)
...
hist(bookModTrain$amount)
hist(bookModTrain$qty)
...
# Checking VIFs
library(faraway)
bookModTrain$Mhistory19Fhistory19 = bookModTrain$Mhistory19*bookModTrain$Fhistory19
bookModTrain$Mhealth35Fhealth35 = bookModTrain$Mhealth35*bookModTrain$Fhealth35
bookModTrain$Fconthist20Mconthist20 = bookModTrain$Fconthist20*bookModTrain$Mconthist20

vif(bookModTrain[,c('recency','frequency','amount','tof','Mgroup','Fgroup','price3mo','qty6mo','price6mo','qty1mo','price12mo','onetothreeflagmo','qty','orderinrecent3mo','orderinrecent6mo','orderinrecent12mo','startedinrecent6mo','startedinrecent12mo','PerSales6mo','amtuponorders','qtyuponorders','amtuponqty','ordersupontof','amtupontof','Mhistory19','Mcartoons5','Mconthist20','Fconthist20','Mhistory19Fhistory19','Mhealth35Fhealth35','Fconthist20Mconthist20')])
...
#New VIFs
vif(bookModTrain[,c('recency','frequency','amount','tof','Mgroup','Fgroup','qty6mo','price6mo','qty1mo','price12mo','qty','orderinrecent3mo','orderinrecent6mo','orderinrecent12mo','startedinrecent6mo','startedinrecent12mo','PerSales6mo','amtuponqty','qtyupontof','Mhistory19','Mcartoons5','Mhistory19Fhistory19','Mhealth35Fhealth35','Fconthist20Mconthist20')])
...
# Checking VIFs
library(faraway)
bookModTrain3$Mhistory19Fhistory19 = bookModTrain3$Mhistory19 * bookModTrain3$Fhistory19
vif(bookModTrain3[,c('frequency','amount','qty3mo','price3mo','amtuponorders','amtuponqty','ordersupontof','amtupontof','qtyupontof','Mhistory19','Fconthist20', 'Mconthist20', 'Mhistory19Fhistory19')])
...
vif(bookModTrain3[,c('frequency','amount','amtuponorders','Mhistory19', 'Mhistory19Fhistory19','amtuponqty')])
```
...
bookMod[(bookMod$id == 5900190) | (bookMod$id == 14158205) ,c('id','amtuponorders','qtyuponorders','amtuponqty')]

#adjust for extreme values 
#change any predicted value larger than 2 times the max amount in the training set (2 data points) to 2 times the max amount of the training set 
...
paste("Sum of squares of error : ", result_ssep)
...
paste("Payoff for top 500 customers ", payoff)
...
paste("Maximum short term profit: $", payoffTable$STP[payoffTable$STP == max(payoffTable$STP)])
...
paste("Payoff for top optimal customers ", payoff2)
